 ...
 */
function vw_add_sub_menu_toggle( $output, $item, $depth, $args ) {
	if ( 0 === $depth && in_array( 'menu-item-has-children', $item->classes, true ) ) {

		// Add toggle button.
		$output .= '<button class="sub-menu-toggle" aria-expanded="false" onClick="twentytwentyoneExpandSubMenu(this)">';
		$output .= '<span class="icon-plus">' . vw_get_icon_svg( 'ui', 'plus', 18 ) . '</span>';
		$output .= '<span class="icon-minus">' . vw_get_icon_svg( 'ui', 'minus', 18 ) . '</span>';
		$output .= '<span class="screen-reader-text">' . esc_html__( 'Open menu', 'twentytwentyone' ) . '</span>';
		$output .= '</button>';
	}
	return $output;
}
add_filter( 'walker_nav_menu_start_el', 'vw_add_sub_menu_toggle', 10, 4 );
 ...
 * @return string
 */
function vw_get_social_link_svg( $uri, $size = 24 ) {
 ...
 */
function vw_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'footer' === $args->theme_location ) {
		$svg = vw_get_social_link_svg( $item->url, 24 );
		if ( ! empty( $svg ) ) {
...
add_filter( 'walker_nav_menu_start_el', 'vw_nav_menu_social_icons', 10, 4 );
 ...
 * @return stdClass
 */
function vw_add_menu_description_args( $args, $item, $depth ) {
...
add_filter( 'nav_menu_item_args', 'vw_add_menu_description_args', 10, 3 );
