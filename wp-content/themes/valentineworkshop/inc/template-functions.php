 ...
 * @return array
 */
function vw_body_classes( $classes ) {
...
add_filter( 'body_class', 'vw_body_classes' );
...
add_filter( 'post_class', 'vw_post_classes', 10, 3 );
 ...
 * @return void
 */
function vw_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'vw_pingback_header' );
 ...
 * @return void
 */
function vw_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'vw_supports_js' );
 ...
 * @return array
 */
function vw_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'vw_comment_form_defaults' );
 ...
 * @return bool
 */
function vw_can_show_post_thumbnail() {
	/**
 ...
 * @since Twenty Twenty-One 1.0
 */
function vw_continue_reading_text() {
 ...
 * @since Twenty Twenty-One 1.0
 */
function vw_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . vw_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'vw_continue_reading_link_excerpt' );
 ...
 * @since Twenty Twenty-One 1.0
 */
function vw_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . vw_continue_reading_text() . '</a></div>';
	}
}

// Filter the content more link.
add_filter( 'the_content_more_link', 'vw_continue_reading_link' );

if ( ! function_exists( 'vw_post_title' ) ) {
	/**
 ...
 * @return string
 */
function vw_get_icon_svg( $group, $icon, $size = 24 ) {
 ...
 * @return string
 */
function vw_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? vw_get_icon_svg( 'ui', 'arrow_right' ) : vw_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? vw_get_icon_svg( 'ui', 'arrow_left' ) : vw_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'vw_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function vw_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'vw_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'vw_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'vw_generate_css' ) ) {
	...
	return vw_generate_css( // @phpstan-ignore-line.
 ...
 */
function vw_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function vw_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
...
add_filter( 'the_password_form', 'vw_password_form', 10, 2 );
 ...
 */
function vw_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'vw_get_attachment_image_attributes', 10, 3 );
